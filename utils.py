...
def get_MiDaS_samples_Real(image_folder, depth_folder, config, specific=None, aft_certain=None):
    ...
    root = '/scratch/f_ndata/G/dataset/RealEstate10K/sorted_1k'
    ...
    for data in tqdm(datas):
        try:
            ...
            tgt_pose_10th = np.array(data['10th'][1:13]).astype(np.float32).reshape(3, 4)
            inss.append(intrinsics)
            src_poss.append(src_pose)
            tgt_pose_1s.append(tgt_pose_5th)
            tgt_pose_2s.append(tgt_pose_10th)
            lines.append(videoname+'/'+first_name)
    ...
    if aft_certain is not None and len(aft_certain) > 0:
        aft_flag = False
    for (id,seq_dir) in tqdm(enumerate(lines)):
        if specific is not None and len(specific) > 0:
            if specific != seq_dir:
                continue
        if aft_certain is not None and len(aft_certain) > 0:
        ...
        #     sdict['int_mtx'][0, :] = sdict['int_mtx'][0, :] / float(W)
        #     sdict['int_mtx'][1, :] = sdict['int_mtx'][1, :] / float(H)
        sdict['ref_pose'] = ref_pose
        sdict['tgt_pose'] = tgt_pose
        # print(tgt_pose)
        sdict['tgts_poses'] = tgts_poses #TODO use this
        sdict['video_postfix'] = config['video_postfix']
        sdict['tgt_name'] = [os.path.splitext(os.path.basename(sdict['depth_fi']))[0]]
        sdict['src_pair_name'] = sdict['tgt_name'][0]
...
def get_MiDaS_samples_MC(image_folder, depth_folder, config, specific=None, aft_certain=None):
    ...
    for data in tqdm(datas):
        try:
            ...
            tgt_pose_10th = np.array(data['5th'][1:13]).astype(np.float32).reshape(3, 4)
            inss.append(intrinsics)
            src_poss.append(src_pose)
            tgt_pose_1s.append(tgt_pose_5th)
            tgt_pose_2s.append(tgt_pose_10th)
